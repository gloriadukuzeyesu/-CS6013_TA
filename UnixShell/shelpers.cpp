...
std::vector<Command> getCommands(const std::vector<std::string> &tokens) {
    ...
    for (int i = 0; i < ret.size(); ++i) {
        ...
        ret[i].execName = tokens[first]; //the name of the executable
        ret[i].argv.push_back(tokens[first].c_str()); //argv[0] = program name
        std::cout << "execName start: " << ret[i].execName << std::endl;
        ret[i].inputFd = 0; // stdin
        ret[i].outputFd = 1; // stdout
        ...
        for (int j = first + 1; j < last; ++j) {
            ...
            if (tokens[j] == ">" || tokens[j] == "<") {
                ...
                if (tokens[j] == "<") {
                    ...
                    ret[i].inputFd = open(tokens[j + 1].c_str(), O_RDONLY); // if i = 0, create a file descriptor with read only flag
                    if (ret[i].inputFd == -1) { // open error check
                    ...
                    ret[i].outputFd = open(tokens[j + 1].c_str(), O_CREAT | O_WRONLY | O_TRUNC, 0666);
                    if (ret[i].outputFd == -1) { // open error check
                        perror("Error with open() syscall");
                        error = true;
                        break;
                    }
        ...
        if (i > 0) {
            ...
            */
            ...
            ret[i - 1].outputFd = pipeFd[1];
            ret[i].inputFd = pipeFd[0];
    ...
    if (error) {
        ...
        for (auto &i: ret) {
            if (i.outputFd != 1) {
                if (close(i.outputFd) == -1) {
                    perror("Error in closing outputFd");
                }
            }
            if (i.inputFd != 0) {
                if (close(i.inputFd) == -1) {
                    perror("Error in the inputFd");
                }
            }
        }
    }
